...
class LangevinCorrector(Corrector):
    ...
    def update_fn(self, x, y, t, *args):
        ...
        for _ in range(self.n_steps):
            ...
            step_size = ((target_snr * noise_norm / grad_norm)
                         ** 2 * 2).unsqueeze(0)
...
@CorrectorRegistry.register(name='ald_with_dnn')
class AnnealedLangevinDynamics_dnn(Corrector):
    ...
    def update_fn(self, x, y, t, *args):
        ...
        target_snr = self.snr
        std = self.sde.marginal_prob(x, y, t, *args)[1]  # std=sigma

        for _ in range(n_steps):
            # when we replace score_fn with ScoreModel.dnn, we should change the way to the same as we call ScoreModel.forward()
            grad = self.score_fn(x, y, t, *args)
            ...
            # return score
            noise = torch.randn_like(x)
            step_size = (target_snr * std) ** 2 * 2  # this is theta
            x_mean = x + step_size[:, None, None,
                                   None] * grad  # xi ` isθ ̊ pxi, σiq
            x = x_mean + noise * torch.sqrt(step_size * 2)[:, None, None, None]
