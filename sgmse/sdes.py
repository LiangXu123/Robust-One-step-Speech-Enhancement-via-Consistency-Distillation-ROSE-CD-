...
class SDE(abc.ABC):
    ...
    def reverse(oself, score_model, probability_flow=False):
        ...
        class RSDE(oself.__class__):
            ...
            def rode_parts(self, x, y, t, *args):
                sde_drift, sde_diffusion = sde_fn(x, y, t, *args)
                score = score_model(x, y, t, *args)
                score_drift = -sde_diffusion[:, None, None, None]**2 * \
                    score * 0.5
            ...
            def rsde_parts(self, x, y, t, *args):
                ...
                diffusion = torch.zeros_like(
                    sde_diffusion) if self.probability_flow else sde_diffusion
                total_drift = sde_drift + score_drift
                return {
                    'total_drift': total_drift, 'diffusion': diffusion, 'sde_drift': sde_drift,
                    'sde_diffusion': sde_diffusion, 'score_drift': score_drift, 'score': score,
...
class SBVESDE(SDE):
    ...
    def add_argparse_args(parser):
        ...
        parser.add_argument("--k", type=float, default=2.6,
                            help="Parameter of the diffusion coefficient. 2.6 by default.")
        parser.add_argument("--c", type=float, default=0.4,
                            help="Parameter of the diffusion coefficient. 0.4 by default.")
    ...
    def sde(self, x, y, t):
        # Table 1
        f = 0.0
        g = torch.sqrt(torch.tensor(self.c)) * \
            self.k**(t)                      # Table 1
        return f, g
    ...
    def _sigmas_alphas(self, t):
        ...
        sigma_bart = torch.sqrt(sigma_T**2 - sigma_t **
                                2 + self.eps)             # below Eq. (9)

        return sigma_t, sigma_T, sigma_bart, alpha_t, alpha_T, alpha_bart

    def _mean(self, x0, y, t):
        sigma_t, sigma_T, sigma_bart, alpha_t, alpha_T, alpha_bart = self._sigmas_alphas(
            t)

        w_xt = alpha_t * sigma_bart**2 / \
            (sigma_T**2 + self.eps)                # below Eq. (11)
        w_yt = alpha_bart * sigma_t**2 / \
            (sigma_T**2 + self.eps)                # below Eq. (11)

        mu = w_xt[:, None, None, None] * x0 + \
            w_yt[:, None, None, None] * y     # Eq. (11)
        return mu
    ...
    def prior_logp(self, z):
        raise NotImplementedError(
            "prior_logp for SBVE SDE not yet implemented!")
