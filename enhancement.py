...
if __name__ == '__main__':
    ...
    parser.add_argument("--corrector", type=str, choices=("ald", "langevin",
                        "none"), default="ald", help="Corrector class for the PC sampler.")
    parser.add_argument("--corrector_steps", type=int,
                        default=1, help="Number of corrector steps")
    ...
    model = ScoreModel.load_from_checkpoint(
        args.ckpt, map_location=args.device, strict=False)
    ...
    print('model.backbone : {}'.format(model.backbone))
    args.N = int(args.N)
    print('args.N : {}'.format(args.N))
    print('model.c_in : {}'.format(model.c_in))
    print('model.c_out : {}'.format(model.c_out))
    print('model.c_skip : {}'.format(model.c_skip))
    print('model.loss_type : {}'.format(model.loss_type))
    print('model.loss_weighting : {}'.format(model.loss_weighting))
    print('model.l1_weight : {}'.format(model.l1_weight))
    print('model.t_eps : {}'.format(model.t_eps))
    print('model.pesq_weight : {}'.format(model.pesq_weight))
    print('model.network_scaling : {}'.format(model.network_scaling))
    print('model.sigma_data : {}'.format(model.sigma_data))
    print('model.num_eval_files : {}'.format(model.num_eval_files))
    ...
    for noisy_file in tqdm(noisy_files):
        ...
        if sr != target_sr:
            y = torch.tensor(
                resample(y.numpy(), orig_sr=sr, target_sr=target_sr))
        ...
        if model.sde.__class__.__name__ == 'OUVESDE':
            if args.sampler_type == 'pc':
                ...
                raise ValueError(
                    f"Sampler type {args.sampler_type} not supported")
            ...
            raise ValueError(
                f"SDE {model.sde.__class__.__name__} not supported")
        ...
        write(join(args.enhanced_dir, filename),
              x_hat.cpu().numpy(), target_sr)
