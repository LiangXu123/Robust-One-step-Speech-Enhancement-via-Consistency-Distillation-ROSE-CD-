...
# Load environment variables (including INPUT_DIR_SIG24)
...
# Use INPUT_DIR_SIG24 environment variable as input directory
DEFAULT_INPUT_DIR="$INPUT_DIR_SIG24"

# Verify INPUT_DIR_SIG24 is set and directory exists
if [ -z "$DEFAULT_INPUT_DIR" ] || [ ! -d "$DEFAULT_INPUT_DIR" ]; then
    echo "❌ Error: INPUT_DIR_SIG24 is not set or directory does not exist."
    exit 1
fi
